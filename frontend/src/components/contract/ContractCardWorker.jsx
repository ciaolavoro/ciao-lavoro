...
export function ContractCardWorker({ contract }) {
    ...
    return (
        <div className={`max-w-md mx-auto my-2 border rounded-lg overflow-hidden p-6 ${getStatusColor(contract.estatus)}`}>
            ...
            <p className="mb-2"><strong>Estado:</strong> <span className={`${getStatusColor(contract.estatus)}`}>{contract.estatus}</span></p>
            ...
            {(contract.estatus === "Negociacion" || contract.estatus === "Aceptado"  || contract.estatus === "Pagado") && (
                ...
                <div className="flex justify-center items-center">
                    {contract.estatus === "En proceso" && (
                        <button className="bg-green-500 hover:bg-orange-700 text-white font-bold py-2 px-4 rounded mt-4 self-center"
